...
class App extends Component {
  ...
  render() {
    let ItemsInCart = this.state.cart.map(item => item.quantity).reduce((a, b) => a + b, 0);
    let total = this.state.cart.map(item => item.price * item.quantity).reduce((a, b) => a + b, 0).toFixed(2);
    let empty = this.state.cart.length === 0 ? 'Cart is empty...' : 'Shopping Cart';
    return (
      <div className='container'>
        <div className='modal' style={this.state.modal ? {display:'flex'} : {display:'none'}}>
          <div className='modal-content'>
            <div className='modal-header'>
              <span className='modal-title'>{empty}</span>
              <div className='modal-close' onClick={() => this.setState({modal:false})}>
            ...
            </div>
            {
              ...
              <div className='modal-product' key={item.id}>
                ...
                <div className='modal-amount'>
                  <button type='button' value={item.id} onClick={this.addToCart}>+</button>
                  <span>{item.quantity}</span>
                  <button type='button' value={item.id} onClick={this.removeFromCart}>-</button>
                </div>
              </div>
            )}
            <div className='modal-footer'>
              <div className='modal-empty-cart' onClick={this.emptyCart} title='Empty cart'>
        ...
        <div className='header'>
          ...
          <div className="search">
            <input value={this.state.search} onChange={this.Search} type="text" placeholder="Type to search... e.g. phone, tablet, pc" maxLength="40"/>
          </div>
            <div className='cart-btn' onClick={() => this.setState({ modal: true })}>
